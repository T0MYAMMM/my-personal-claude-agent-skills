---
name: web-scraping
description: This skill activates for web scraping development. It proactively discovers APIs via traffic interception, recommends optimal strategy (traffic interception/sitemap/API/DOM scraping/hybrid), and implements iteratively in Python using httpx, playwright-python, and crawlee-python.
license: MIT
---
...
- "Productionize this scraper" or "Deploy to Cloud Run / Google Batch"
...
**See**: `strategies/httpx-vs-browser-test.md` for the httpx viability assessment
...
**See**: `strategies/httpx-vs-browser-test.md` for validation methodology

**Step 3a: Validate CSS selectors**
- For each httpx/BeautifulSoup-based method: confirm the selector matches actual HTML
...
```
- 200 → httpx viable, no browser needed for accessible endpoints
...
After reconnaissance report is accepted, implement scraper iteratively in Python.
...
6. Add robustness (retries, logging, rate limiting)

**See**: `workflows/implementation.md` for complete Python implementation patterns

### PRODUCTIONIZATION (on request)

Convert scraper to production-ready GCP service.

**Activation triggers**: "Productionize this", "Deploy to Cloud Run", "Run on Google Batch"

**Core Pattern**:
1. Package as Docker container (Python + Playwright Chromium)
2. Expose CLI entrypoint (Click)
3. Choose deployment: Cloud Run (HTTP/scheduled) or Google Batch (parallel jobs)
4. Wire GCS output and Secret Manager credentials

**See**: `workflows/productionization.md` for complete workflow
...
| httpx vs Browser | Three-way test + early exit | `strategies/httpx-vs-browser-test.md` |
...
| Find sitemaps | `robots.txt` → sitemap XML | `strategies/sitemap-discovery.md` |
| Filter sitemap URLs | `re.search()` patterns | `reference/regex-patterns.md` |
| Discover APIs | Traffic capture (automatic) | `strategies/api-discovery.md` |
| DOM scraping | playwright-python + DevTools | `strategies/dom-scraping.md` |
| HTTP scraping | httpx + BeautifulSoup4 | `strategies/httpx-scraping.md` |
| Crawlee Python | BeautifulSoupCrawler / PlaywrightCrawler | `strategies/httpx-scraping.md` |
| Hybrid approach | Sitemap + API | `strategies/hybrid-approaches.md` |
| Handle blocking | playwright-stealth + curl_cffi + proxies | `strategies/anti-blocking.md` |
...
| Productionize | Dockerfile + Cloud Run / Google Batch | `workflows/productionization.md` |
...
async def main() -> None:
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=10.0) as client:
        resp = await client.get("https://example.com/sitemap.xml")
...
See `examples/sitemap-basic.py` for complete example.
...
async def main() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        for pid in product_ids:
            resp = await client.get(f"https://api.example.com/products/{pid}")
            resp.raise_for_status()
            print(resp.json())
            await asyncio.sleep(0.1)  # 10 req/s
```

See `examples/api-scraper.py` for complete example.
...
See `examples/hybrid-sitemap-api.py` for complete example.

---

## Directory Navigation

This skill uses **progressive disclosure** — detailed information is in subdirectories, loaded only when needed.

### Workflows (Implementation Patterns)

- `workflows/reconnaissance.md` — Phase 1 interactive reconnaissance (CRITICAL)
- `workflows/implementation.md` — Iterative Python implementation patterns
- `workflows/productionization.md` — GCP Cloud Run / Google Batch deployment

### Strategies (Deep Dives)

- `strategies/framework-signatures.md` — Framework detection lookup tables (Phase 0/1)
- `strategies/httpx-vs-browser-test.md` — httpx vs Browser decision test with early exit
...
- `strategies/api-discovery.md` — Finding and using APIs (httpx)
- `strategies/dom-scraping.md` — DOM scraping via playwright-python + DevTools bridge
- `strategies/httpx-scraping.md` — HTTP-only scraping (httpx + BeautifulSoup4 / crawlee-python)
- `strategies/hybrid-approaches.md` — Combining strategies
- `strategies/anti-blocking.md` — Multi-layer anti-detection (stealth, playwright-stealth, curl_cffi, proxies)
- `strategies/session-workflows.md` — Session recording, HAR export, replay

### Examples (Runnable Python)

- `examples/playwright-scraper.py` — Browser + traffic interception basics
- `examples/sitemap-basic.py` — Sitemap scraper (crawlee-python)
- `examples/api-scraper.py` — Pure API approach (httpx)
- `examples/hybrid-sitemap-api.py` — Combined approach
- `examples/iterative-fallback.py` — Try API → BS4 → Playwright → crawl

### Reference (Quick Lookup)

- `reference/report-schema.md` — Intelligence report format (Sections 1-7 + self-critique)
- `reference/proxy-tool-reference.md` — Proxy-MCP tool reference (80+ tools)
...
### 5. Python-First Production Code
When productionizing:
- Use Python (`httpx`, `playwright-python`, `crawlee-python`)
- Package in Docker (Python 3.11 slim + Playwright Chromium)
- Deploy to GCP Cloud Run (HTTP/cron) or Google Batch (parallel jobs)
- Credentials via Secret Manager, output to GCS
