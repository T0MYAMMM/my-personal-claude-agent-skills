...
Reusable fictional clients. Pick one for consistency across a session series, or append a new one
(use the template at the bottom) so future sessions in the same arc stay in voice. All personas are
**fictional composites** — never identifiable real people.

Persona `id`s are lowercase, used by `scaffold_session.py --persona <id>`.
...
- **Best fits**: which modalities suit this persona.
```
