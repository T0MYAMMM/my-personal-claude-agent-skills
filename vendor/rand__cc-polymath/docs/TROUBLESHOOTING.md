...
**Common mistake**: Using old paths like `../skills` instead of `../skills`. See Issue #4 if you have both directories.
...
**Why this matters**: The plugin system at `../` is the source of truth. Manual installations at `../skills` will be stale and cause confusion.
...
- Old paths like `../skills` no longer work
...
**Why this changed**: The plugin system requires skills to live in `../` instead of `../skills`. This allows multiple plugins to coexist without conflicts.
...
- **Report bug**: [GitHub Issues](https://github.com/rand/cc-polymath/issues) with:
  - Steps to reproduce
  - Output of `bash ../scripts/verify-install.sh`
...
1. **Verify everything works**: `bash ../scripts/verify-install.sh`
